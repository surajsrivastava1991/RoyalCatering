page 50085 "Custumised General Journal"
{
        ...
        area(content)
        {
            group(Control120)
            {
                ...
                field(CurrentJnlBatchName; CurrentJnlBatchName)
                {
                    ...
                    begin
                        CurrPage.SaveRecord();
                        LookupCustumisedBatchName(CurrentJnlBatchName, Rec);
            ...
            group(Control30)
            {
                ...
                fixed(Control1901776101)
                {
                    ...
                    group("Total Debit")
                    {
                        Caption = 'Total Debit';
                        Visible = IsSimplePage;
                        field(DisplayTotalDebitf; DisplayTotalDebit())
                        {
                            ApplicationArea = Basic, Suite;
                            Caption = 'Total Debit';
                            Editable = false;
                    ...
                    group("Total Credit")
                    {
                        ...
                        Visible = IsSimplePage;
                        field(DisplayTotalCreditf; DisplayTotalCredit())
                        {
                            ApplicationArea = Basic, Suite;
                            Caption = 'Total Credit';
                            Editable = false;
    ...
    local procedure LookupCustumisedBatchName(var CurrentJnlBatchName: code[10]; VAR GenJnlLine: Record "Gen. Journal Line")
    var
        GenJnlBatch: Record "Gen. Journal Batch";
    begin
        COMMIT();
        ...
        GenJnlBatch.setfilter("Approval Status", '%1|%2|%3', GenJnlBatch."Approval Status"::Open,
        GenJnlBatch."Approval Status"::canceled, GenJnlBatch."Approval Status"::Rejected);
    ...
    var
        lGenBatch: Record "Gen. Journal Batch";
        lGenJnlTemp: Record "Gen. Journal Template";
    begin

        if not lGenBatch.get("Journal Template Name", "Journal Batch Name") then
            exit;
        lGenBatch.delete;
        CurrentJnlBatchName := '';
        FILTERGROUP := 2;
